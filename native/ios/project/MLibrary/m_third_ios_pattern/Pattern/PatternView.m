...
static CGFloat const nodeSide = 14.0f;
...
- (void)layoutSubviews {
    ...
    NSLog(@"totlaWidth : %f", totalWidth);
    NSLog(@"totlaHeight : %f", totalHeight);
    
    CGFloat availableWidth = totalWidth - (3 * nodeSide);
    CGFloat availableHeight = totalHeight - (3 * nodeSide);
    CGFloat nodeSeperation = availableWidth / 2 - 8.0f;
    CGFloat vNodeSeperation = availableHeight / 2 - 8.0f;
    CGFloat marginpadding = nodeSeperation / 2;
    
    NSLog(@"availableWidth : %f", availableWidth);
    NSLog(@"nodeSeperation : %f", nodeSeperation);
    NSLog(@"marginpadding : %f", marginpadding);
    
    NSDictionary *views = @{@"node1":self.node1, @"node2":self.node2, @"node3":self.node3, @"node4":self.node4, @"node5":self.node5,
                            @"node6":self.node6, @"node7":self.node7, @"node8":self.node8, @"node9":self.node9, @"drawingView":self.drawingView};
    NSDictionary *metrics = @{@"padding":@(nodeSeperation), @"vPadding":@(vNodeSeperation)};
    
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-[node1]-(padding)-[node2]-(padding)-[node3]-|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-[node4]-(padding)-[node5]-(padding)-[node6]-|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-[node7]-(padding)-[node8]-(padding)-[node9]-|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-[node1]-(vPadding)-[node4]-(vPadding)-[node7]-|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-[node2]-(vPadding)-[node5]-(vPadding)-[node8]-|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-[node3]-(vPadding)-[node6]-(vPadding)-[node9]-|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
    
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"|[drawingView]|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[drawingView]|"
                                                                 options:kNilOptions
                                                                 metrics:metrics
                                                                   views:views]];
...
- (void)addAnimations {
    ...
    self.pushBehaviour1 = [[UIPushBehavior alloc] initWithItems:@[self.node1, self.node2, self.node3]
                                                           mode:UIPushBehaviorModeContinuous];
    [self.pushBehaviour1 setAngle:-M_PI_2];
    [self.pushBehaviour1 setMagnitude:3.0];
    
    self.pushBehaviour2 = [[UIPushBehavior alloc] initWithItems:@[self.node4, self.node5, self.node6]
                                                           mode:UIPushBehaviorModeContinuous];
    [self.pushBehaviour2 setAngle:-M_PI_2];
    [self.pushBehaviour2 setMagnitude:2.3];
    
    self.pushBehaviour3 = [[UIPushBehavior alloc] initWithItems:@[self.node7, self.node8, self.node9]
                                                           mode:UIPushBehaviorModeContinuous];
...
#pragma mark Add Gesture Recongnizer
...
- (BOOL)shouldAddPoint:(CGPoint)newPoint {
    ...
    CGPoint previousPoint = [[self.pointsArray lastObject] CGPointValue];
    return (fabs(newPoint.x - previousPoint.x) > permittedMaxOffset || fabs(newPoint.y - previousPoint.y) > permittedMinOffset);
}
