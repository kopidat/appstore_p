//
//  HttpCustomNetworkManager.m
//  SKIMP_DemoApp
//
//  Created by Sungoh Kang on 2021/06/21.
//

#import "HttpCustomNetworkManager.h"
...
@interface HttpCustomNetworkManager ()

@end

@implementation HttpCustomNetworkManager

/*
 * @brif 비동기 데이터 통신 요청 함수, *구현 필수
 * @param viewController    현재 viewcontroller
 * @param path              전문코드
 * @param data              request data string
 * @param networkOption     네트워크 옵션 object
 * @param delegate          result를 넘겨 받을 delegate
...
- (void)processWithViewCtrl:(UIViewController *)viewController trcode:(NSString *)path data:(NSString *)data networkoption:(PPNetworkOption *)networkOption delegate:(id<PPNetworkManagerDelegate>)delegate userinfo:(NSArray *)userInfo {
    ...
    if (self.resourceUpdateURI && networkOption.restAction) {
        ...
        if (provider.type == PPHttpBasePostProvide || provider.type == PPHttpBasePostProvide) {
    ...
    // 비동기 통신이기 때문에 파라메터로 받았던 데이터들을 통신이 종료 될때 까지 유지 시키기 위해 provider에 포함시킴
    userInfo = userInfo == nil ? @[] : userInfo;
    NSArray *arguments = @[viewController, path, data, networkOption,
                           delegate, userInfo];
    
    // Arguments 를 UserData 로 저장
    // 비동기 통신이기 때문에 파라메터로 받았던 데이터들을 통신이 종료 될때 까지 유지 시키기 위해 provider에 UserData 로 설정
 ...
 * @param encoding              response string charset encoding
 * @param provider_userdata     저장해둔 parameters
 */
- (void)didFinishHttpProvider:(id)provider header:(NSDictionary *)header body:(NSData *)body encoding:(NSStringEncoding)encoding userdata:(NSArray *)userData {
    PPDebug(@"http reponse header : %@", header);
    ...
    if (self.resourceUpdateURI && networkOption.restAction) {
        ...
        dataType = [header stringForKey:@"user_data_type"];
...
/*
    @brief resource update download 시에 사용되는 method
    @remarks
